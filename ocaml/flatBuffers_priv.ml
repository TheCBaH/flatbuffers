module FlatBuffers = struct
  ...
  module ByteBuffer = struct
    ...
    let writeInt64 t offset value =
      ...
      let b345 =
        Int64.to_int
          (Int64.logand (Int64.shift_right_logical value 24) 0xFFFFFFL)
      in
      write_uint24 t (offset + 3) b345 ;
      let b67 =
        Int64.to_int
          (Int64.logand (Int64.shift_right_logical value 48) 0xFFFFFFL)
      in
  ...
  let _ =
    assert (
      do_test ByteBuffer.readUint16 ByteBuffer.writeUint16 0xFFFF "\xFF\xFF" )

  let _ =
    assert (
      do_test ByteBuffer.readInt16 ByteBuffer.writeInt16 (-32768) "\x00\x80" )

  let _ =
    assert (
      do_test ByteBuffer.readUint16 ByteBuffer.writeUint16 0xF0FF "\xFF\xF0" )
  ...
  module Builder = struct
    ...
    let finish t ?(file_identifier = "") root_table =
      if String.length file_identifier > 0 then
        prep t t.minalign
          ~additional_bytes:(_SIZEOF_INT + _FILE_IDENTIFIER_LENGTH) ;

...
let check_float msg ~expected actual =
  if Float.is_nan expected
  then check msg (Float.is_nan actual)
  else if Float.is_infinite expected
  then check_eq msg ~expected actual
...
let check_float_array msg ~expected actual =
  ...
  Array.iteri
    (fun i e -> check_float (Printf.sprintf "%s[%d]" msg i) ~expected:e actual.(i))
    expected
;;
...
let verify_monsterdata ?(check_doubles = true) (type a) (prim : a Primitives.t) (buf : a) =
  ...
  if check_doubles then check_float "pos.test1" ~expected:3.0 (Vec3.test1 buf pos);
...
let test_offset64_roundtrip () =
  ...
  let verify ?(check_doubles = true) (type a) (prim : a Primitives.t) (buf : a) =
    ...
    if check_doubles
    then check_float "far_struct[0].b" ~expected:3.14 (LeafStruct.b buf s0);
    ...
    if check_doubles
    then check_float "big_struct[2].b" ~expected:3.0 (LeafStruct.b buf bs2)
  in
...
let () =
  ...
  Printf.printf "Melange: Verification\n%!";

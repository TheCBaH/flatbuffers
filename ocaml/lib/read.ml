...
let[@inline] get_val (type a b) (t : a tag) (prim : b Primitives.t) (b : b) (i : int) : a =
  ...
  | TRef64 -> i + Primitives.get_uoffset64 prim b i
  | TStruct _ -> i
;;
...
let[@inline] read_table_ref64 p b i n =
  let i' = get_indirect p b i n in
  if i' < 0 then invalid_arg "required field not set" else get_val TRef64 p b i'
;;

let[@inline] read_table_opt_ref64 p b i n =
  let i' = get_indirect p b i n in
  if i' < 0 then i' else get_val TRef64 p b i'
;;
...
let[@inline] to_list_vec64 t p b i =
  List.init (length_vec64 p b i) (fun j -> unsafe_get_vec64 t p b i j)
;;

let[@inline] to_array_vec64 t p b i =
  Array.init (length_vec64 p b i) (fun j -> unsafe_get_vec64 t p b i j)
;;

let[@inline] to_seq_vec64 t p b i =
  let len = length_vec64 p b i in
  let rec aux j () =
    if j < len
    then (
      let x = unsafe_get_vec64 t p b i j in
      Seq.Cons (x, aux (j + 1)))
    else Seq.Nil
  in
  aux 0
